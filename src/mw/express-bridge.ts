import { DataProvider, Remult, IdEntity, Entity, Repository, Fields, JsonDataProvider, ErrorInfo,ClassType,actionInfo,DataApi, allEntities, getEntityKey, AllowedForInstance, DataApiRequest, DataApiResponse, jobWasQueuedResult, serializeError, Action, queuedJobInfoResponse, registerActionsOnServer,registerEntitiesOnServer } from 'remult';
import { JsonEntityFileStorage } from "./JsonEntityFileStorage.ts";
...
export function remultMiddlewareBase(
    ...
    let bridge = new ExpressBridge(app, new inProcessQueueHandler(options.queueStorage), options.initRequest!,
        //@ts-ignore
        dataProvider);
    ...
    return Object.assign(app, {
        getRemult: (req: any) => bridge.getRemult(req),
        openApiDoc: (options: { title: string }) => bridge.openApiDoc(options),
        addArea: (x: any) => bridge.addArea(x)
    });
}
...
export interface GenericResponse {
    status(arg0: number): { json: (x: any) => void };
    statusCode: number;
    json(data: any): void;
